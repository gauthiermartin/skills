...
## Research root (local configuration)

All research topic dirs live under the exocortex vault:
`RESEARCH_ROOT="$HOME/Documents/Vaults/exocortex/03-Resources/Research"` — a topic lives at
`$RESEARCH_ROOT/<topic-slug>/` (no `research-` prefix on the dir name). Set `RESEARCH_ROOT`
at the top of any bash you run. To locate existing topics, scan `"$RESEARCH_ROOT"/*/index.yaml`.
...
| **init** | No matching research dir exists and the user wants a new research topic | Create `$RESEARCH_ROOT/<topic-slug>/`, then run **append** or **deep** by the same rules (the deep-research gate applies if sources/links were provided). | ~1-10 min append · ~10-40+ min deep |
...
2. Locate a matching research dir by explicit path, topic slug, or scanning
   `"$RESEARCH_ROOT"/*/index.yaml` for a semantic topic match.
...
Use the same locator logic as Step 0 (path provided / topic slug / scan `$RESEARCH_ROOT` / ask if multiple).
...
```bash
uv run --script ${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-research/scripts/build_index_md.py --research-dir "<research_dir>"
```
...
|---|---|---|
| `obsidian` | Obsidian vault search (a research source) | Warn: "⚠️ Obsidian CLI unavailable — skipping your vault as a source. Enable the Obsidian CLI, put `obsidian` on PATH, or set `OBSIDIAN_CLI` to the CLI executable. See the `anvil-obsidian-cli` skill." Drop Obsidian from `available_clis`; continue with other sources. |
| `readwise` | Readwise library + feed search | Not configured in this setup (intentionally skipped) — degrade silently, no warning needed. Continue. |
| `nlm` | NotebookLM search | Not configured in this setup (intentionally skipped) — degrade silently. Set `notebook_ids = []`; skip Step 3b's auth check. Continue. |
...
> ❌ No research sources available. Enable the Obsidian CLI (see the `anvil-obsidian-cli`
> skill) or include a seed URL/PDF, then re-run.
...
3. **Generic web URLs** (e.g., `https://example.com/article`) — any `http(s)://` link that is **not** one of the recognized special origins below (not a vault path, not a Readwise reference, not a GitHub repo, not a NotebookLM URI, not a YouTube URL, and not a `.pdf`): **fetch it with `curl` and strip the HTML to readable text** using only preinstalled tools (`curl` + `python3` stdlib — no extra dependency). This handles normal, server-rendered HTML sites; it does **not** clear bot walls, CAPTCHAs, JS-only rendering, or paywalls — `WebFetch` is the fallback for those (see below).
   ...
   curl -fsSL --compressed --max-time 30 \
     ...
     > "$(mktemp -d)/research-scrape-<slug>.md"
   ```
...
The GitHub pipeline never clones *into* the research directory; clones go to a reusable `.github-cache/` placed **as a sibling of the research dir** (i.e. in the research dir's parent — for `$RESEARCH_ROOT/<slug>/` the cache lands at `$RESEARCH_ROOT/.github-cache/`, a dot-dir Obsidian ignores). This keeps reusable clones next to the research dirs without surfacing them in the vault. Only curated spec docs land in the final research dir.
...
1. **Parse targets** — run the parser over the brain dump AND any markdown files the user referenced (e.g., an outline.md). The parser extracts repo-relative file paths + line ranges and groups them by parent directory (one "module" per dir):
   ```bash
   uv run --script ${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-research/scripts/github_parse_targets.py \
     --repo "<repo_url>" \
...
2. **Shallow-clone** the repo into the cache. Pass `--research-dir` so the cache lands as a sibling of the research dir (its parent), not under working memory:
   ```bash
   uv run --script ${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-research/scripts/github_clone.py \
     --repo "<repo_url>" \
...
5. **Emit exactly ONE seed entry per repo**, keyed to the ARCHITECTURE doc:
   ...
   - `github_files`: **union** of every file path referenced across all modules (from the parser output) — used by `/anvil-obsidian-distill-research` for matching, not for indexing individual docs. Empty list in global mode.
...
1. **Check presence, then authentication.** NotebookLM is not configured in this setup
   (intentionally skipped). If `nlm` was MISSING in Step 0.5 — the expected case — skip this
   entire step silently: set `notebook_ids = []` and continue. If `nlm` is present, run
   `nlm login --check`. If that fails, log a warning that NotebookLM will be skipped for
   this run (auth expired/absent), set `notebook_ids = []`, and continue.
...
1. **Deduplicate** the round's subagent outputs via script:
   ```bash
   uv run --script ${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-research/scripts/dedup_findings.py \
...
1. **Compute the research-dir path**. Default: `$RESEARCH_ROOT/<topic-slug>/`. Override if the user specified a different output path. Create the v4 layout up front:
   ```bash
...
2. **Spawn the Builder Subagent**. Read `agents/builder.md` and pass it as the subagent prompt, along with:
   ...
   - `skill_dir`: `${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-research` (absolute path)
...
1. **PDFs** — for any source where `origin == "pdf"` (user-dropped) or where the original is a `.pdf` URL, the Builder will have already invoked `extract_pdf.py` to extract markdown + images and preserve the original at `raw/assets/<slug>/original.pdf`. If somehow not done, run it now:
   ```bash
   uv run --script ${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-research/scripts/extract_pdf.py \
...
2. **Images** — for every successful raw markdown file, download referenced images to local assets and rewrite refs:
   ```bash
   uv run --script ${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-research/scripts/download_assets.py \
...
2. **Run `build_index_yaml.py`** to emit the canonical index:
   ...
   uv run --script ${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-research/scripts/build_index_yaml.py \
...
3. **Run `build_index_md.py`** to regenerate the Obsidian-readable view:
   ```bash
   uv run --script ${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-research/scripts/build_index_md.py \
     --research-dir "<research_dir>"
   ```
...
- **How to use it**:
  ...
  - For programmatic access: pass `index.yaml` to `/obsidian-skills:anvil-obsidian-research` (query mode), which handles progressive disclosure
- **Next steps suggestion**: "Run `/anvil-obsidian-lint-research` to health-check the wiki, or `/anvil-obsidian-render-research marp <topic>` to export a slide deck."

## Important notes

**Search strategy — Obsidian**: Use the `obsidian` CLI for search operations: `obsidian search query="<terms>" limit=20`. Once you have the file paths from search results, read the files directly using the Read tool. See the `anvil-obsidian-cli` skill for full command reference. **Guard:** only run this if `obsidian` is in `available_clis` (presence checked in Step 0.5 via `command -v obsidian`); if it's missing, skip the vault as a source — never let `command not found` abort the search.

**Search strategy — Readwise**: *Not configured in this setup — skip this source. The section below is kept for future re-enablement (install the `readwise` CLI and restore its usage skill).* Use the `readwise` CLI (not the MCP tool). **Guard:** only run this if `readwise` is in `available_clis` (presence checked in Step 0.5 via `command -v readwise`); if it's missing, skip Readwise as a source — never abort on `command not found`. Readwise splits into two areas and research must cover both:
...
**Search strategy — NotebookLM**: *Not configured in this setup — skip this source. The section below is kept for future re-enablement (install the `nlm` CLI and restore its usage skill).* Use the `nlm` CLI. Key commands for research:
