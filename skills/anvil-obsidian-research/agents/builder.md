...
- **research_dir**: Target directory (e.g. `$RESEARCH_ROOT/<slug>/`, where `RESEARCH_ROOT="$HOME/Documents/Vaults/exocortex/03-Resources/Research"`). Create the v4 layout if missing: `mkdir -p "<research_dir>/raw/assets" "<research_dir>/wiki"`. Final files (`index.yaml`, `index.md`, `log.md`, `raw/`, `wiki/`) sit directly under this dir; scratch JSONs (the augmented `-with-uris.json` temps you write below) live alongside them and get cleaned up in the orchestrator's Step 7.
...
- **skill_dir**: Absolute path to the `anvil-obsidian-research` skill directory (in case you need to invoke a script — currently you don't).
...
```bash
uv run --script "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-research/scripts/youtube_extract_transcript.py" \
