...
Use the `readwise` CLI via Bash (not the MCP tool). (Readwise is not configured in this setup — this step is skipped when `readwise` is absent from `available_clis`.)
