...
A fifth notional format ("table") was considered but rejected: tables are best embedded inside `comparison` wiki pages, which `/obsidian-skills:anvil-obsidian-research` already produces via its `comparison_writer`. If you want a table, ask for a comparison.
...
Locate `research_dir` the same way `/obsidian-skills:anvil-obsidian-research` (query mode) and `/anvil-obsidian-lint-research` do.
...
- **All output goes into `wiki/renders/`.** Renders are wiki artifacts; they compound. Never write anywhere outside the research dir.
