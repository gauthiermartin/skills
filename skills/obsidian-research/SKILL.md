...
> ❌ No research sources available. Enable the Obsidian CLI (see the `obsidian-cli`
...
5. **Emit exactly ONE seed entry per repo**, keyed to the ARCHITECTURE doc:
   ...
   - `github_files`: **union** of every file path referenced across all modules (from the parser output) — used by `/obsidian-distill-research` for matching, not for indexing individual docs. Empty list in global mode.
...
- **Next steps suggestion**: "Run `/obsidian-lint-research` to health-check the wiki, or `/obsidian-render-research marp <topic>` to export a slide deck."
