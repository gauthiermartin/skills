---
name: anvil-obsidian-lint-research
description: Health-check a research directory produced by /obsidian-skills:anvil-obsidian-research. Runs seven checks — orphan sources, missing entity/concept hubs, missing comparison candidates, broken wikilinks, stale claims, contradictions, and open-question synthesis. Outputs a report; edits where safe (broken-link flags, open-question append, contradiction surfacing); flags-only otherwise. Always user-triggered, never automated. Trigger when the user says things like "lint my research", "health check my research", "check the wiki", "audit my research dir", "what's wrong with my wiki", "find orphans / contradictions / stale claims".
metadata:
...
Same logic as `/obsidian-skills:anvil-obsidian-research` (query mode):
...
```bash
SKILL_DIR="${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/anvil-obsidian-lint-research"
RD="<research_dir>"
...
| missing-hubs | NO | flag — user can /obsidian-skills:anvil-obsidian-research it or accept the absence |
| missing-comparisons | NO | flag — user can `/anvil-obsidian-render-research comparison <a> <b>` if they want |
...
- For missing hubs: run /obsidian-skills:anvil-obsidian-research with the concept name, OR accept the absence
- For missing comparisons: run /anvil-obsidian-render-research comparison <a> <b>
