...
def build_data():
    ...
    changes, re-run /obsidian-render-research to regenerate.
    """
    # ...
...
5. Hard-code the data inside `build_data()`. The wiki snapshot you read is now embedded in the script — that's the *point*: the PNG is reproducible from this script alone, even if the wiki changes later. If the user wants a re-render against fresh data, they re-run `/obsidian-render-research`.
